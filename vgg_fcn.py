...
class FCN:
    ...
    def build(self, rgb, train=False, num_classes=20, random_init_fc8=False,
        ...
        if random_init_fc8:
            ...
            self.score_fr = self._fc_layer(self.fc7, 'score_fr',
                                            num_classes=num_classes, relu=False)
        ...
        self.pred_with_softmax = tf.nn.softmax(self.upscore32, name='pred_softmax')

        self.pred_up = tf.argmax(self.upscore32, axis=3, name='pred_up')
    ...
    def _fc_layer(self, bottom, name, num_classes=None, relu=True, debug=False):
        with tf.variable_scope(name) as scope:
            ...
            if debug:
                debug = tf.Print(bias, [tf.shape(bias)],
                                        message='Shape of {}'.format(name),
                                        summarize=4, first_n=1)
            return bias
    ...
    def _score_layer(self, bottom, name, num_classes):
        with tf.variable_scope(name) as scope:
            ...
            # Apply bias
            conv_biases = self._bias_variable([num_classes], constant=0.8)
            bias = tf.nn.bias_add(conv, conv_biases)

            _activation_summary(bias)
            return bias

    def _upscore_layer(self, bottom, shape, num_classes, name, debug,
                        ksize=4, stride=2):
    ...
    def _summary_reshape(self, fweight, shape, num_new):
        ...
        VGG16 produces 1000 classes. Most semantic segmentation task require
        much less classes. This reshapes the original weights to be used in
        a fully-convolutional layer which produces num_new classes. To
        archive this the average (mean) of n adjanced classes is taken.
...
def _variable_summaries(var, logger):
    ...
    if not tf.get_variable_scope().reuse:
        name = var.op.name
        logger.info('Creating Summary for: {}'.format(name))
...
def _gradient_summary(grad, var):
    # if grad is not None:
    tensor_name = var.op.name
    tf.summary.histogram(tensor_name + "/gradient", grad)

def _regularization_summary(var):
    # if var is not None:
    tensor_name = var.op.name
    tf.summary.histogram(tensor_name, var)
    regularization = tf.nn.l2_loss(var)
    tf.add_to_collection("reg_loss", regularization)
